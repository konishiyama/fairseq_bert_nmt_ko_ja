...
class TransformerEncoderLayerBase2(nn.Module):
    """Encoder layer block.
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False, bert_gate=True, return_fc=False):
        ...
        self.self_attn = self.build_self_attention(self.embed_dim, cfg)
        # 
        bert_out_dim = cfg.bert_out_dim
        self.bert_attn = MultiheadAttention(
            self.embed_dim, cfg.encoder_attention_heads,
...
class TransformerEncoderLayer2(TransformerEncoderLayerBase2):
...
class TransformerDecoderLayerBase2(nn.Module):
    """Decoder layer block.
...
# backward compatible with the legacy argparse format
class TransformerDecoderLayer2(TransformerDecoderLayerBase2):
    def __init__(
        self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False
    ):
        super().__init__(
